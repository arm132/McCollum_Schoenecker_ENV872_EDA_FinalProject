...
```{r Prelim Wrangling, eval=F, echo=F}
...
```{r SPP and Event Wrangling, eval=T, echo=F}
# DURHAM
...
![Figure 1. Scatterplot showing species richness of each county for each month of the 10-year time period.](./Output/EDA_SppRichness_Plot.png)
![Figure 2. Scatterplot showing number of unique sampling events (USEs) in each county for each month of the 10-year time period.](./Output/EDA_EventCount_Plot.png)
Since the eBird data included latitude and longitude for every observation, the other component of our exploratory analysis included visualizing the location of each observation geospatially (Figure 3). This plot shows that the distribution of sampling locations was not evenly distributed across the region and that observations were concentrated around the urban centers of each county (Durham, Chapel Hill, and Raleigh).  
...
![Figure 3. Map of Durham, Orange, and Wake counties with points symbolizing each sampling location in the eBird dataset.](./Output/EDA_geospatial_plot.png)
\newpage
...
![Figure 5a. Time series decomposition for species richness in Durham County from 2010 to 2019.](./Output/Durham_spp_ts_decomp.png)
![Figure 5b. Time series decomposition for species richness in Orange County from 2010 to 2019.](./Output/Orange_spp_ts_decomp.png)
![Figure 5c. Time series decomposition for species richness in Wake County from 2010 to 2019.](./Output/Wake_spp_ts_decomp.png)

## Question 1b: Was there a clear trend in the presence of species of concern across the decade? 

For each of the three counties, the number of SGCN observed in the final year of the period (2019) was noticeably higher than the number observed in the first year of the period (2010; Figure 6). However, none of the counties exhibited a constant increase in SGCN count across the entire decade, suggesting that further analysis is necessary to fully understand the trend in these data.
...
![Figure 7. Time series decomposition for event count aggregated across all three counties from 2010 to 2019.](./Output/RT_event_ts_decomp.png)

![Figure 8a. Time series decomposition for event count in Durham County from 2010 to 2019.](./Output/Durham_event_ts_decomp.png)
![Figure 8b. Time series decomposition for event count in Orange County from 2010 to 2019.](./Output/Orange_event_ts_decomp.png)
![Figure 8c. Time series decomposition for event count in Wake County from 2010 to 2019.](./Output/Wake_event_ts_decomp.png)

In order to complete these time series analyses in R, we utilized the zoo (Zeilies and Grothendeick 2005) and tseries (Trapletti and Hornik 2021) packages to construct the time series objects and the trend (Pohlert 2020) and Kendall (McLeod 2011) packages to perform the seasonal Mann-Kendall tests.    
